...
<div class="container">
    ...
    <form id="user_input_form" name="user_input" action="{{ url('post/'.$post->id) }}" method="post"
        ...
        <div class="form-group">
            ...
            <textarea id="sequence_body" class="form-control @if ($errors->has('sequence_body')) is-invalid @endif"
                name="sequence_body" rows="8" required>{{ old('sequence_body', $post->sequence_body) }}</textarea>
        ...
        <button id="submit_images" type="submit" name="submit" class="row ml-1 btn btn-outline-success"><i
                class="fas fa-paper-plane"></i>{{ __('投 稿') }}</button>
    </form>
